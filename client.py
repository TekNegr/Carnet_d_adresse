...
temp_contact_list = [User("Billy","BOB","@mail","123kk"), User("Klem","Ent","@mail.com","123kk")]
...
class Home_Client(Client):
    def __init__(self, user: User):
        ...
        self.contact_frame_maker(temp_contact_list)
       
        
    def contact_frame_maker(self, contact_list):
        self.contact_frame = tk.Frame(self.Mainframe)
        self.contact_frame.grid(row=0, column=0)
        for i in range(len(contact_list)):
            self.contact_frame.rowconfigure(i,weight=1)
            text_contact = tk.Text(self.contact_frame.grid(row=i, column=0),height=1, width=30)
            text_contact.insert(tk.END, contact_list[i].name)
            text_contact.config(state=tk.DISABLED)
            text_contact.grid(row=i,column=0)
...
user = User("Henin","N1","1@mail.com","0")
